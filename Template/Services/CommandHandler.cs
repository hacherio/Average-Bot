...
namespace Template.Services
{
    public class CommandHandler : InitializedService
    {
        ...
        {
            if (arg3.MessageId != 842967072321437696) return;

            if (arg3.Emote.Name != "😩") return;

            var role = (arg2 as SocketGuildChannel).Guild.Roles.FirstOrDefault(x => x.Id == 842967762666389525);
            await (arg3.User.Value as SocketGuildUser).AddRoleAsync(role);
        }

        private async Task OnJoinedGuild(SocketGuild arg)
        {
            await arg.DefaultChannel.SendMessageAsync("Thank you for inviting Average Bot!");
        }
